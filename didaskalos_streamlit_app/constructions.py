# Clause-level constructions read off the dependency annotation the treebanks already carry but nothing downstream ever used: each entry in SYNTAX_ENTRIES becomes one section of the syntax reference at the end of the book, a table of the categories attested in the chosen corpus with a few cited examples, and an instance the classifier cannot place is dropped rather than guessed at, since the reference needs a handful of examples per row and not every instance, so a rule can discard most of its candidates and still fill the table.
...
# Treebanks number homograph lemmas inconsistently, Gorman writing the modal particle an1 (390 tokens) beside a plain an (51), so an unnormalized count reads a third of the truth.
...
# AGDT hangs the clause verb below its subordinator, which is an AuxC, while UD hangs the subordinator below the verb as a mark; measured: ei 975 child / 215 parent in AGDT, hina 281 parent / 1 child in PROIEL.
HOP_DIRECTION = {"agdt-xml": "child", "conllu": "parent"}

# Authors whose text is verse: random draws from the verse treebanks produced elided hoste, vocative interjections and tragic word order, none of which reads as an example of anything to a beginner.
...
# ei introduces indirect questions ("whether") as well as conditions, and after these verbs it nearly always does; without the guard, "ho d' ephasken, ei polla eie" was offered as a past general condition.
...
# How many attested examples one reference row prints, and the floor below which a row is reported as unattested rather than illustrated.
EXAMPLES_PER_ROW = 3
MIN_ATTESTED = 3

# Deliberately the same vocabulary parse_form_features decodes into, so a category label renders through the existing feat_* locale keys rather than a second set; spelled out here rather than imported because didaskalos_pipeline imports this module.
...
class ClauseIndex:
    # The tree lookups every classifier needs, built once per frame: the joins are vectorized and the per-node maps cover only verbs and modal particles, since a dict over every token costs more than parsing the treebank does.
    def __init__(self, df: pd.DataFrame, fmt: str = "agdt-xml"):
        ...
        # postag and relation are categorical in combined_df to keep the frame small, and filling a categorical with a category it does not have is an error, so both go back to plain objects first.
        ...
        # Only verbs are ever looked up by node: every caller tests for one, and a miss falls through to the same answer a non-verb would have given.
        ...
        # The treebanks split eite into ei + enclitic -te, so the disjunction is only visible as a dependent of the ei.
    ...
    def clause_verb(self, sid: str, node_id: str):
        # The verb of the clause a subordinator introduces: AGDT hangs it below the subordinator and UD above, so trying below first and then above answers for either, which also covers a mixed selection of corpora in one book.
    ...
    def apodosis_verb(self, sid: str, subordinator_id: str, protasis_id: str):
        # The verb the protasis hangs off, found by walking up from it and stepping over the subordinator, which sits above the protasis verb in AGDT and below it in UD.
        node = protasis_id
        for _ in range(6):
    ...
    def head_verb(self, sid: str, node_id: str):
        # The verb the whole clause depends on: a conditional's apodosis, or the matrix verb of a subordinate clause.
...
# The six types a textbook teaches, or None; ei carrying an an dependent is ean written apart, which Perseus does and which otherwise loses roughly sixty per cent of the future more vivid conditions.
def _conditional_type(protasis_tag: str, apodosis_tag: str, is_ean: bool, apodosis_an: bool):
    ...
    # A prescription with an imperative apodosis is not a general condition, and letting it through labelled one instance of it a present general.
...
# One entry per reference section: "trigger" selects the tokens worth looking at, "classify" places each in a category or returns None to drop it.
SYNTAX_ENTRIES: dict[str, dict] = {
    ...
    # Kept apart from hina: after verbs of striving hopos takes a future indicative and the clause is one of effort, not purpose, so merging the two would hide the very contrast the section describes.
...
def catalog_author(file_name, document_id=None):
    # Only the curated catalog answers here; a work it does not know is treated as prose rather than dropped, since the verse treebanks are all catalogued.
...
def reference_rows(
    ...
    # Every classified instance of one construction, with the provenance an example line needs; length and prose gates are applied here so a caller cannot accidentally print a forty-word line of tragedy.
    ...
    for row in triggers.itertuples(index=False):
        ...
        if kind == "modal":
            ...
            # eite ... eite is "whether ... or", a disjunction filed under ei; it is not a conditional and reads as a poor example of one.
...
def all_reference_rows(df: pd.DataFrame, fmt: str = "agdt-xml", author_lookup=None):
    # One index serves every entry; rebuilding it per section cost more than the whole detection does.
